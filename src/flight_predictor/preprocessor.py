...
class Preprocessor:
    ...
    COLUMNS_TO_DROP = ["ch_code", "num_code", "time_taken", "dep_time", "arr_time", "stop"]
    ...
    def _clean_duration(self, df):
        df["duration_minutes"] = (
              df["time_taken"].str.extract(r"(\d+)h")[0].fillna(0).astype(int) * 60
            + df["time_taken"].str.extract(r"(\d+)m")[0].fillna(0).astype(int)
        )
        return df

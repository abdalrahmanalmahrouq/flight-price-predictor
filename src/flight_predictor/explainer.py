...
class Explainer:
    ...
    def summary_bar(self):
        self._check_fitted()
        shap.summary_plot(self._shap_values, plot_type="bar", show=False)
        self._save("shap_summary_bar.png")

    def summary_dot(self):
        self._check_fitted()
        shap.summary_plot(self._shap_values, show=False)
        self._save("shap_summary_dot.png")

    def waterfall(self, sample_index: int = 0):
        self._check_fitted()
        shap.plots.waterfall(self._shap_values[sample_index], show=False)
